...
bool PluginSpecPrivate::readMetaData(const QJsonObject &pluginMetaData)
{
    ...
    const QString strtest = QString::fromUtf8(QJsonDocument(pluginMetaData).toJson());
    QJsonValue value;
    ...
    if (value.toString() == "org.qt-project.Qt.QDesignerCustomWidgetInterface") {
        value = pluginMetaData.value(QLatin1String("className"));
        if (value.isUndefined())
            return reportError(msgValueMissing("className"));
        if (!value.isString())
            return reportError(msgValueIsNotAString("className"));
        name = value.toString();
        
        value = pluginMetaData.value(QLatin1String("version"));
        if (value.isUndefined())
            return reportError(msgValueMissing("version"));
        if (!value.isDouble())
            return reportError(msgValueIsNotAString("version"));
        version = QString::number(value.toDouble());
        return true;
    }
