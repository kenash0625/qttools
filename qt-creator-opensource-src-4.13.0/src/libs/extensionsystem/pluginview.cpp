...
QList<QString> PluginView::disabledPlugins()
{
    QList<QString> ldisabledplugins;
    for(int chidx=0;chidx<m_model->rootItem()->childCount();chidx++)
    {
        auto chi=m_model->rootItem()->childAt(chidx);
        for(int chidx2=0;chidx2<chi->childCount();chidx2++)
        {
            auto chi2=chi->childAt(chidx2);
            PluginItem *item=static_cast<PluginItem*>(chi2);
            if(item&&!item->m_spec->isEnabledBySettings())
            {
                ldisabledplugins.append(item->m_spec->name());
            }
        }

    }
    return ldisabledplugins;
}
...
bool PluginView::setPluginsEnabled(const QSet<PluginSpec *> &plugins, bool enable)
{
    ...
    emit pluginSettingsChanged(nullptr);
    return true;
}
