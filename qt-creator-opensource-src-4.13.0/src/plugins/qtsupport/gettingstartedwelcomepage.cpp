...
protected:
    void clickAction(const ListItem *item) const override
    {
        ...
        if (exampleItem->isSingleUIFile)
        {
            emit poWidget->ExampleClickAction(exampleItem->description);
        }
        else if (exampleItem->isVideo)
...
ExamplesPageWidget::ExamplesPageWidget(bool isExamples)
    : m_isExamples(isExamples)
{
    m_exampleDelegate = new ExampleDelegate; 
    m_exampleDelegate->poWidget = this;
    m_gridModel = new GridProxyModel; 
    ...
    m_examplesModel->updateExamples3();
...
int ExamplesPageWidget::bestColumnCount() const
{
    return qMax(1, width() / (GridProxyModel::GridItemWidth + GridProxyModel::GridItemGap));
}

void ExamplesPageWidget::resizeEvent(QResizeEvent *ev) 
{
    QWidget::resizeEvent(ev);
    m_gridModel->setColumnCount(bestColumnCount());
}

void ExamplesPageWidget::onTagClicked(const QString &tag)
{
