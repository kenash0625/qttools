...
void SnippetProvider::decorateEditor2(TextEditorWidget *editor, const QString &groupId,void*p)
{
    for (const SnippetProvider &provider : g_snippetProviders) {
        if (provider.m_groupId == groupId && provider.m_editorDecorator2)
            provider.m_editorDecorator2(editor,p);
    }
}
...
void SnippetProvider::registerGroup2(const QString &groupId, const QString &displayName,
                                     EditorDecorator2 editorDecorator)
{
    SnippetProvider provider;
    provider.m_groupId = groupId;
    provider.m_displayName = displayName;
    provider.m_editorDecorator2 = editorDecorator;
    g_snippetProviders.append(provider);
}