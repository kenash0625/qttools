...
*/
void SyntaxHighlighter::setFormat(int start, int count, const QTextCharFormat &newformat)
{
    Q_D(SyntaxHighlighter);
    ...
    for (int i = start; i < end; ++i) {
        ...
        QTextCharFormat newf2=newformat;
        if(ofont.bold())
        {
            QFont newfont = newformat.font();
            newfont.setBold(true);
            newf2.setFont(newfont);
        }
        d->formatChanges[i] = newf2;
    }
}
