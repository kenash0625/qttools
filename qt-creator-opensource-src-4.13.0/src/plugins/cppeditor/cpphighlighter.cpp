...
CppHighlighter::CppHighlighter(void *poSkipHighlight,QTextDocument *document)
    :SyntaxHighlighter(document)
{
    this->poSkipHighlight = poSkipHighlight;
...
void CppHighlighter::highlightBlock(const QString &text)
{
    ...
    QList<CtrlProperty> *poSkip = (QList<CtrlProperty> *) this->poSkipHighlight;
    if (poSkip) {
        QList<int> rmidx;
        int idx = 0;
        int blkpos = currentBlock().position();
        for (auto &prop : *poSkip) {
            if (blkpos <= prop.spos && (blkpos + text.size()) >= prop.epos) {
                for (size_t i = prop.spos - blkpos; i < prop.spos - blkpos + prop.epos - prop.spos + 1;
                     i++) {
                    QTextCharFormat ofmt = format(i);
                    QFont ofont = ofmt.font();
                    ofont.setBold(true);
                    ofmt.setFont(ofont);
                    setFormat(i, 1, ofmt);
                }
                rmidx.push_front(idx);
                //QTextCharFormat fmt;
                //QFont font;
                //font.setBold(true);
                //fmt.setFont(font);
                //setFormat(prop.spos - blkpos, prop.epos - prop.spos + 1, fmt);
            }
            idx++;
        }
        //for (auto idx : rmidx)
    ...
    QString strfind=QStringLiteral(");//LTCTEST225");
    int idx=text.indexOf(strfind);
    if (idx!=-1) {
        QString strObj, strProp, strValue;
        auto lst=text.mid(idx).split(QStringLiteral(" "),Qt::SkipEmptyParts);
        if(lst.size()>3)
        {
            strObj=lst.at(1);
            strProp=lst.at(3);
            //int spos=text.indexOf(lst.at(2))+lst.at(2).size();
            if (strProp == "text") {
                auto tk = tokens.at(10);
                strValue = text.mid(tk.utf16charsBegin(), tk.utf16chars());
                emit SetObjProp(strObj, strProp, strValue);
            }
            else if (strProp == "flat") {
                auto tk = tokens.at(4);
                strValue = text.mid(tk.utf16charsBegin(), tk.utf16chars());
                emit SetObjProp(strObj, strProp, strValue);
            }
