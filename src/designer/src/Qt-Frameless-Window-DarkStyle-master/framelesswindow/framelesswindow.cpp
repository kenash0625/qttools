...
void FramelessWindow::setMenu(QWidget *w,int menuheight)
{
  ui->windowTitlebar->setFixedHeight(menuheight);
  ui->horizontalLayout->insertWidget(1,w,1);
}

void FramelessWindow::setButtons(bool showMax, bool showMin, bool showRestore, bool showClose)
{
  ui->restoreButton->setVisible(showRestore);
  ui->maximizeButton->setVisible(showMax);
  ui->minimizeButton->setVisible(showMin);
  ui->closeButton->setVisible(showClose);
}
...
void FramelessWindow::setWindowIcon(const QString &ico) {
  ui->icon->setPixmap(QPixmap(ico));
  ui->icon->setFixedSize(150,50);
