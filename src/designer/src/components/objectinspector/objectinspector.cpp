...
void ObjectInspector::ObjectInspectorPrivate::selectObjectByName(const QString &name)
{
    for(auto &item:m_model->m_model)
    {
        if(item.object()->objectName()==name)
        {
...
void ObjectInspector::selectObjectByName(const QString &s)
{
   m_impl->selectObjectByName(s);
}
