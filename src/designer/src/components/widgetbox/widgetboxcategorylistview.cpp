...
void WidgetBoxCategoryEntryDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    QRect pxrect=option.rect;
    pxrect.setWidth(option.rect.width()/4);
    painter->drawPixmap(pxrect,QPixmap(":/qt-project.org/designer/images/gear.png"));
    QRect txtrect=option.rect;
    txtrect.setLeft(option.rect.width()/3);
    txtrect.setWidth(option.rect.width()-txtrect.left());
    painter->setPen(Qt::black);
    QString s=index.data().toString();
    painter->drawText(txtrect,s);
    QPen pen(Qt::darkGray,2);
    painter->drawLine(option.rect.bottomLeft(),option.rect.bottomRight());
}

QSize WidgetBoxCategoryEntryDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
