...
QString designerApplicationName = "可视化开发工具";
QString designerDisplayName = "可视化开发工具";
QString designerWarningPrefix = "可视化开发工具： ";
...
void QDesigner::showErrorMessage(const QString &message)
{
    ...
    const QString qMessage =
        message.right(message.size() - (designerWarningPrefix.size()));
...
QDesigner::ParseArgumentsResult QDesigner::parseCommandLineArguments()
{
    ...
    //const QString localSysName = "zh_CN";
    const QString localSysName = QLocale::system().name();
    const QString filePath=QCoreApplication::applicationDirPath()+"/../translations";
    QScopedPointer<QTranslator> designerTranslator(new QTranslator(this));
    if (designerTranslator->load(QStringLiteral("designer_") + localSysName, filePath)) {
        installTranslator(designerTranslator.take());
        QScopedPointer<QTranslator> qtTranslator(new QTranslator(this));
        if (qtTranslator->load(QStringLiteral("qt_") + localSysName, filePath))
            installTranslator(qtTranslator.take());
        {

            QScopedPointer<QTranslator> qtTranslator(new QTranslator(this));
            if (qtTranslator->load(QStringLiteral("qtcreator_") + localSysName, filePath))
                installTranslator(qtTranslator.take());
        }
        {

            QScopedPointer<QTranslator> qtTranslator(new QTranslator(this));
            if (qtTranslator->load(QStringLiteral("qtbase_") + localSysName, filePath))
                installTranslator(qtTranslator.take());
        }
    }
    m_homeTemplateSelection=new HomeTemplateSelection();
    connect(m_homeTemplateSelection->ui->pbNewProject,&QPushButton::clicked,[&](){
        m_workbench->frmlessWnd->showMaximized();
    });
    connect(m_homeTemplateSelection,&HomeTemplateSelection::ExampleClickAction,[&](const QString&fileName){
         m_workbench->openForm2(fileName);
        m_workbench->frmlessWnd->showMaximized();
    });
    m_homeTemplateSelection->framelessWindow.show();
    setFont(QFont("Microsoft YaHei UI",15));
    ...
    emit m_workbench->m_actSignalSlotWnd->triggered(false);
    m_workbench->frmlessWnd->hide();
    ...
    for (auto action : recentActions) {
        ...
        item->setData(Qt::UserRole+1,action->iconText());
        m_homeTemplateSelection->ui->lstRecent->addItem(item);
    }
    connect(m_homeTemplateSelection->ui->lstRecent, &QListWidget::itemDoubleClicked,
            [&](QListWidgetItem *item){
                m_workbench->openForm2(item->data(Qt::UserRole+1).toString());
                m_workbench->frmlessWnd->showMaximized();
            });
    return result;
}

void QDesigner::designerMainwindow(Options&options)
{
