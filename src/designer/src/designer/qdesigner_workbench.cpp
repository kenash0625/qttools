    ...
    m_globalMenuBar(new QMenuBar)
{
    ...
    m_core->m_actViewcppCode=m_actionManager->m_viewCppCodeAction;
    ...
    for (int i = 0; i < QDesignerToolWindow::StandardToolWindowCount; i++) {
        if(i==QDesignerToolWindow::ResourceEditor||i== QDesignerToolWindow::ActionEditor)
        {
            continue;
        }
        ...
        if (QAction *action = toolWindow->action()) {
            ...
            if(i==QDesignerToolWindow::SignalSlotEditor)
            {
                m_actSignalSlotWnd=action;
                action->setText("连接关系绑定");
            }
        }
    ...
    m_state = StateUp;
    QAction *aboutPluginsAction = new QAction(tr("插件列表展示"), this);
    ...
    QMenu*menuPlugin=m_globalMenuBar->addMenu("插件管理");
    menuPlugin->addAction(aboutPluginsAction);
    menuPlugin->addAction(m_actSignalSlotWnd);
//    connect(acttmp,&QAction::triggered,m_actSignalSlotWnd,&QAction::triggered);
    QMenu*menuStyle=m_globalMenuBar->addMenu("界面风格切换");
    auto styleFunc = [](const QString&str) {
        QFile file(str);//(":/qss/lightblue.css");
        if(file.open(QIODevice::ReadOnly))
        {
        QString qss=file.readAll();
        QString pallet=qss.mid(20,7);
        qApp->setPalette(QColor(pallet));
        qApp->setStyleSheet(qss);
        }
    };

    QAction*a1= menuStyle->addAction("浅蓝色",[=](){styleFunc(":/qss/lightblue.css");})
    ,*a2=menuStyle->addAction("科技黑",[=](){styleFunc(":/qss/blacksoft.css");})
    ,*a3=menuStyle->addAction("绿色",[=](){styleFunc(":/qss/flatgray.css");});
    ...
    frmlessWnd=new FramelessWindow;
    QMainWindow*mw=new QMainWindow(frmlessWnd);
    int menuheight=50;
    QString strmenustyle="min-height: "+QString::number(menuheight)+"px;min-width: "+QString::number(menuheight)+"px;font: 15px \"Microsoft YaHei UI\"";
    m_globalMenuBar->setStyleSheet(strmenustyle);
    mw->setMenuBar(m_globalMenuBar);
    mw->takeCentralWidget();
    frmlessWnd->setMenu(mw,menuheight);
    frmlessWnd->setWindowIcon(":/qt-project.org/designer/designer.ico");
    frmlessWnd->setContent(m_dockedMainWindow);
}
...
void QDesignerWorkbench::openForm2(const QString &fileName)
{
    m_actionManager->openForm2(fileName);
}
